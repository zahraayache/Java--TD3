...
//	}
	...
	public static void main(String[] args) {
		...
		Circle circle = new Circle(new Point(2,3),2);
		...
		cs[1] = c2;
		cs[2] = c2;
		System.out.println(Circle.contains(new Point(1,2), cs));
	}
	...
	public static String contains(Point p, Circle...circles) {
		int count = 0;
		if(circles.length >0) {
			for (Circle c : circles) {
				if ((p.getX() - c.center.getX()) * (p.getX() - c.center.getX()) +
				        (p.getY() - c.center.getY()) * (p.getY() - c.center.getY()) <= c.radius * c.radius)
				        count++;
